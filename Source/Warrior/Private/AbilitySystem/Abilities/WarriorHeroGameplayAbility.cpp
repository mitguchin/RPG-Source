...
void UWarriorHeroGameplayAbility::SafeAddMappingContext(UInputMappingContext* Context, int32 Priority) const
{
	if (!Context) return;

	AWarriorHeroController* PC = const_cast<UWarriorHeroGameplayAbility*>(this)->GetHeroControllerFromActorInfo();
...
void UWarriorHeroGameplayAbility::SafeRemoveMappingContext(UInputMappingContext* Context) const
{
	if (!Context) return;
	...
	if (UEnhancedInputLocalPlayerSubsystem* Subsys =
		ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LP))
	{
		Subsys->RemoveMappingContext(Context);
	}
}
